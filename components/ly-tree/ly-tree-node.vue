	...
	<view ref="node" name="LyTreeNode" v-show="node.visible" class="ly-tree-node" :class="{ 
	 ...
	 role="treeitem" @tap.stop="handleClick">
		<view class="ly-tree-node__content" :class="{
			...
			{{node.label}}
			<text v-if = "node.data.hasOwnProperty('count')" class = "ly-tree-node__count">
				({{node.data.count}}条)
			</text>
			</text>
			<ly-checkbox v-if="checkboxVisible || radioVisible" class="ly-checkbox" :type="checkboxVisible ? 'checkbox' : 'radio'"
			 :checked="node.checked" :indeterminate="node.indeterminate" :disabled="!!node.disabled" @check="handleCheckChange(!node.checked)" />
		</view>
	...
	export default {
		...
		methods: {
			...
			handleCheckChange(checked) {
				...
				// console.log(this.tree.store.getCheckedNodes())
				this.$nextTick(() => {
...
<style>
	...
	.ly-tree-node__expand-icon.expanded {
		...
		width: 44upx;
		height: 44upx;
		background: url(~@/static/images/splice.png) no-repeat center center;
